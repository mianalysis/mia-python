...
class ImageWrapper:
    def __init__(
        self, name: str, raw_image
    ):  # To do (raw_image could be ImagePlus, ImgPlus or DataArray)
        ...
        if isinstance(raw_image, JImagePlus):
            ...
            self._image = Image(
                name=name,
                da_img=da_img,
                dpp_xy=calibration.pixelWidth,
                dpp_z=calibration.pixelDepth,
                spatial_units=calibration.getXUnit(),
                frame_interval=calibration.frameInterval,
                temporal_units=calibration.getTimeUnit(),
            )
    ...
    def convertImageToObjects(
        ...
        objs: Objs = self._image.convertImageToObjects(
            coordinate_set_factory.getPythonCoordinateSetFactory(),
            output_objects_name,
            single_object,
        )
    ...
    def convertImageToSingleObjects(
        ...
        objs: Objs = self._image.convertImageToSingleObjects(
            coordinate_set_factory.getPythonCoordinateSetFactory(),
            output_objects_name,
            blackBackground,
        )
    ...
    def show(self, title: str, lut: JLUT | None, normalise, display_mode, overlay):
        print(
            "ImageWrapper: LUT -> Colormap conversion in show.  For now, setting no LUT"
        )
...
class ImageFactoryWrapper:
    ...
    def create(
        self, name: str, raw_image
    ) -> ImageWrapper:  # To do (raw_image could be ImagePlus, ImgPlus or DataArray)
