...
class Volume:
    def __init__(
        ...
        self._coordinate_set: CoordinateSet = (
            coordinate_set_factory.createCoordinateSet()
        )
    ...
    def getMeanCentroid(self, pixel_distances: bool, match_xy: bool) -> Point:
        ...
        if pixel_distances:
            ...
            return np.array(
                (
                    x_mean * self.getDppXY(),
                    y_mean * self.getDppXY(),
                    z_mean * self.getDppZ(),
                )
            )
    ...
    def createNewVolume(
        self, coordinate_set_factory: CoordinateSetFactory, example_volume: Volume
    ) -> Volume:
        return Volume(
            coordinate_set_factory=coordinate_set_factory,
    ...
    def getAsTightImageWithBorders(
        self, image_name: str, border_widths: List[List[int]]
    ) -> Image:
        ...
        tight_im: Image = createImage(
            ...
            n_frames=n_frames,
            d_type=np.uint8,
            dpp_xy=self.getDppXY(),
            dpp_z=self.getDppZ(),
            spatial_units=self.getSpatialUnits(),
            frame_interval=1,
            temporal_units="",
        )

        points: Points = self.getCoordinateSet().getPoints().astype(int)
        tight_im.putAllPixels(
            vals=np.ones(self.size(), dtype=np.float32) * 255,
            x=points[:, 0] - x_offs,
            y=points[:, 1] - y_offs,
            z=points[:, 2] - z_offs,
        )
    ...
    def getX(
        self, pixel_distances: bool
    ) -> npt.NDArray[np.float32] | npt.NDArray[np.uint32]:
        if pixel_distances:
    ...
    def getZ(
        ...
        if pixel_distances:
            if match_xy:
                return (
                    self._coordinate_set.getPoints()[:, 2]
                    * self.getDppZ()
                    / self.getDppXY()
                )
            else:
    ...
    def getCentroidSeparation(
        self, volume2: Volume, pixel_distances: bool, force_2D: bool
    ) -> float:  # To do
    ...
    def getSlice(self, slice: int) -> Volume:
        ...
        slice_volume: Volume = Volume(
            self.getCoordinateSetFactory(),
            width,
            height,
            n_slices,
            dpp_xy,
