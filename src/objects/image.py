...
class Image:
    ...
    def putAllPixels(
        ...
        c: npt.NDArray[np.int32] | None = None,
        z: npt.NDArray[np.int32] | None = None,
        t: npt.NDArray[np.int32] | None = None,
    ):
    ...
    def convertImageToObjects(
        self,
        coordinate_set_factory: CoordinateSetFactory,
        output_objects_name: str,
        single_object: bool,
    ) -> Objs:
        return self.convertImageToObjectsGeneral(
            coordinate_set_factory, output_objects_name, single_object, True
        )

    def convertImageToSingleObjects(
        self,
        coordinate_set_factory: CoordinateSetFactory,
        output_objects_name: str,
        ...
        return self.convertImageToObjectsGeneral(
            coordinate_set_factory, output_objects_name, True, blackBackground
        )

    def convertImageToObjectsGeneral(
        self,
        coordinate_set_factory: CoordinateSetFactory,
        output_objects_name: str,
        single_object: bool,
                    ...
                    for x in range(width):
                        for y in range(height):
                            ...
                            if single_object:
                                image_ID = (
                                    1
                                    if (black_background and image_ID != 0)
                                    or (not black_background and image_ID == 0)
                                    else 0
                                )
                            ...
                            if image_ID != 0:
                                if image_ID not in final_IDs:
                                    final_IDs[image_ID] = (
                                        output_objs.getAndIncrementID()
                                    )

                                out_ID = final_IDs[image_ID]
                                output_objs.createAndAddNewObjectIfMissing(
                                    coordinate_set_factory, out_ID
                                )
...
def createImage(
    ...
    return Image(
        image_name,
        DataArray(
            data=np_arr, coords=coords, dims=dim_names, name=image_name, attrs=attrs
        ),
        dpp_xy=dpp_xy,
        dpp_z=dpp_z,
        spatial_units=spatial_units,
